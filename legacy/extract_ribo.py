...
from mylib.sd import run_fs


GTF_PATH='../reference/genomic.gtf'
FNA_PATH='../reference/GCF_000092025.1_ASM9202v1_genomic.fna'
gtf= GTF(GTF_PATH, FNA_PATH)
...
pathC='/fs/ess/PAS2967/dengyw144/S21_Five/Ribo-seq/TE/result-C.tsv'
pathE='/fs/ess/PAS2967/dengyw144/S21_Five/Ribo-seq/TE/result-E.tsv'
pathF='/fs/ess/PAS2967/dengyw144/S21_Five/Ribo-seq/TE/result-F.tsv'
DESEQ2_PATH='../Ribo-seq/TE'
df1 = pd.read_csv(pathC, sep="\t", index_col=0)
df2=pd.read_csv(pathE, sep="\t", index_col=0)
df3=pd.read_csv(pathF, sep="\t", index_col=0)

def get_significant_genes(df, t_lfc=0, padj=0.05):
    up = df[(df['padj'] <= padj) & (df['log2FC_TE(Control vs DrugTreated)'] >= t_lfc)].index.tolist()
    down = df[(df['padj'] <= padj) & (df['log2FC_TE(Control vs DrugTreated)'] <= -t_lfc)].index.tolist()
    return up, down
...
with open(os.path.join(DESEQ2_PATH, 'AND_CEF.txt'), 'w') as f:
    f.write('up:\n')
    
    def write_id_product(gene_list):
        tmp=[f"{g}\t{run_fs(gtf.gene_to_tir(g, -30, 2))[0]}\t{gtf.id2protein(g)}" for g in gene_list]
        f.write('\n'.join(tmp))
...
print(os.path.join(DESEQ2_PATH, 'CEF_down.pdf'))
plt.clf()
# quit()

with open(os.path.join(DESEQ2_PATH, 'CEF.txt'), 'w') as f:
    f.write('C:\n')
